...
export async function POST(request) {
  ...
  try {
    ...
    const key = `team/${year}/${month}/${randomUUID()}.${IMAGE_EXTENSIONS[actualType]}`;
    ...
    return Response.json(
      {
        ...
        url,
        key,
        name: String(file.name || "Team photo").slice(0, 255),
      },
      { status: 201 },
    );
  } catch (error) {
    console.error("POST /api/uploads/team", error);
    return Response.json(
...
export async function DELETE(request) {
  ...
  try {
    ...
    const deleted = await deleteR2TeamImage(body.url);
    return Response.json({ ok: true, deleted });
  } catch (error) {
    console.error("DELETE /api/uploads/team", error);
    return Response.json(
      { error: "Failed to remove team image." },
      { status: 500 },
    );
  }
}
