...
export async function POST(request) {
  ...
  try {
    ...
    const key = `content/${year}/${month}/${randomUUID()}.${IMAGE_EXTENSIONS[actualType]}`;
    ...
    return Response.json(
      {
        ...
        url,
        key,
        name: String(file.name || "Content image").slice(0, 255),
      },
      { status: 201 },
    );
  } catch (error) {
    console.error("POST /api/uploads/content", error);
    return Response.json(
...
export async function DELETE(request) {
  ...
  try {
    ...
    const deleted = await deleteR2ContentImage(body.url);
    return Response.json({ ok: true, deleted });
  } catch (error) {
    console.error("DELETE /api/uploads/content", error);
    return Response.json(
      { error: "Failed to remove content image." },
      { status: 500 },
    );
  }
}
